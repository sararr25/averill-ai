...
- **Credentials:** async scrypt with random salts; renderer snapshots omit credentials/evidence internals. Temporary generated passwords appear once in the admin receipt, with an explicit Copy action; they are not persisted as plaintext or sent to Nebius. No invitation emails are sent. No password reset/self-service recovery is implemented: save demo credentials. This is local authentication on one Mac; local files remain readable/editable to the OS account, so no production multi-tenant security or multi-device service is claimed.
